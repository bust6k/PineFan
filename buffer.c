...
void buf_write(Buffer* b, char c) {
  if (b->nalloc == (b->len + 1)) realloc_body(b);
  b->body[b->len++] = c;
}

char* buf_read(Buffer* b) { return strdup(b->body); }

char buf_read_ch(Buffer* b) { return b->body[b->rc++]; }

char* buf_read_str(Buffer* b) {
  int i = 0;
  char* line = malloc(sizeof(char) * strlen(b->body));
  while (b->body[i] != '\n') line[i] = b->body[i];

  return line;
}

void buf_unread(Buffer* b, int delta) { b->rc -= delta; }
void buf_append(Buffer* b, char* s, int len) {
  for (int i = 0; i < len; i++) buf_write(b, s[i]);
}
...
char* quote_cstring_len(char* p, int len) {
  Buffer* b = make_buffer();
  for (int i = 0; i < len; i++) print(b, p[i]);
  return buf_body(b);
}
